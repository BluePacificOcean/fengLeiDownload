  ...
  return {
    init: function(path, onready) {
      ...
      if (auth_str && auth_str.indexOf('token:') == 0) {
       ...
       if (jsonrpc_interface.indexOf("ws") == 0 && WebSocket) {
        jsonrpc_protocol = "ws"
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "websocket 错误. 您可能需要重新刷新此页面才能重新启动。");
          ws_callback = {};
        };
    ...
    unpause: function(gids) {
      ...
      ARIA2.batch_request("unpause", gids,
        function(result) {
          ...
          if (error.length == 0) {
            main_alert("alert-info", "已开始任务", 1000);
            ARIA2.refresh();
          } else {
            main_alert("alert-error", error.join("<br />"), 3000);
          }
    ...
    remove: function(gids) {
      ...
      ARIA2.batch_request("remove", gids,
        function(result) {
          ...
          if (error.length == 0) {
            main_alert("alert-info", "删除成功", 1000);
            ARIA2.refresh();
          } else {
            main_alert("alert-error", error.join("<br />"), 3000);
          }
    ...
    change_options: function(gid, options) {
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          ...
          main_alert("alert-info", "配置更新成功", 1000);
        }
      );
    },
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "暂停所有任务。 请等待联系 BitTorrent 跟踪器等操作。", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "删除了所有已完成/错误/已删除的下载任务。", 2000);
        }
      );
    },
      ...
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          if (!result.result) {
            ...
            main_alert("alert-success", "修改配置成功", 2000);
          }
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "自上次更新以来，任务列表已更改。 单击“刷新”按钮以更新任务列表。");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
